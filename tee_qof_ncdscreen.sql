...
(SELECT hos_target from
 (select person.hospcode , count(distinct person.pid) as hos_target from person  
           ...
           and (TIMESTAMPDIFF(YEAR,person.birth,@bdg_date) between 35 and 59) group by person.hospcode ) as t
...
(SELECT hos_doit from
          (select person.hospcode,count(distinct(person.pid)) as hos_doit from ncdscreen  inner join person on ncdscreen.hospcode = person.hospcode and ncdscreen.pid = person.pid 
           where person.discharge = '9' and person.typearea in ('1', '3') and person.nation ='099' and date_serv between @start and @end 
           and (TIMESTAMPDIFF(YEAR,person.birth,@bdg_date) between 35 and 59)  group by person.hospcode) as r
